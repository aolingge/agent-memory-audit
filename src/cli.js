...
function usage() {
  console.log(`agent-memory-audit v${VERSION}

Usage:
  agent-memory-audit --path FILE_OR_DIR
  agent-memory-audit FILE_OR_DIR --markdown
  agent-memory-audit FILE_OR_DIR --sarif > results.sarif
  agent-memory-audit FILE_OR_DIR --annotations
  agent-memory-audit FILE_OR_DIR --redact
...
try {
  ...
  console.error(`agent-memory-audit: ${error.message}`);
  process.exit(2);
}
